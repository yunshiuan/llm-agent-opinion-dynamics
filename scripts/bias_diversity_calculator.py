...
#     37, 38, 39, 40, 41
#     ]
n_topics = len(prompt_versions)
for version in prompt_versions:
    for base in prompt_bases:
        if "reverse" in base:
            continue
        prompt = f"v{version}_{base}"
        out_name = f"topic_results/v{version}_reflective.txt"
        ...
        with open(out_name, "a") as f:
            ...
            diversity = np.round(np.std(final_opinion_profile)/np.sqrt(n_topics), 2)
            bias_list.append(bias)
            diversity_list.append(diversity)

            f.write(f"Bias: {np.round(np.mean(bias_list), 2)}\n")
